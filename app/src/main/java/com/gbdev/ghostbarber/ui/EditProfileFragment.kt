package com.gbdev.ghostbarber.ui
...
import com.gbdev.ghostbarber.databinding.FragmentEditProfileBinding
...
class EditProfileFragment : Fragment() {
    ...
    private fun loadUserProfile() {
        ...
        firestore.collection("users").document(userId).get()
            .addOnSuccessListener { document ->
                if (document.exists()) {
                    val fullUsername = document.getString("username") ?: ""
                    if (fullUsername.startsWith("Mr. Drac ")) {
                        binding.titleTextView.visibility = View.VISIBLE
                        binding.titleTextView.text = "Mr. Drac"
                        binding.usernameEditText.setText(fullUsername.removePrefix("Mr. Drac "))
                    } else {
                        binding.titleTextView.visibility = View.GONE
                        binding.usernameEditText.setText(fullUsername)
                    }
    ...
    private fun saveProfile() {
        val userId = auth.currentUser?.uid ?: return
        val editableUsername = binding.usernameEditText.text.toString()
        val title = if (binding.titleTextView.visibility == View.VISIBLE) "Mr. Drac" else ""
        val fullUsername = if (title.isNotEmpty()) "$title $editableUsername" else editableUsername
